...
# Environment flags pulled from .env
environment = os.getenv("ENVIRONMENT", "")
# CUSTOM_GUILD should be set when running in custom environment. Keep as string for checks
...
class Client(commands.AutoShardedBot):
    ...
    async def _load_views(self):
        ...
        if environment == "custom":
            if not guildid:
                print("[❌] CUSTOM_GUILD not defined in .env; skipping view filtering.")
            else:
                try:
                    filter["guild"] = int(guildid)
                except ValueError:
                    print("[❌] CUSTOM_GUILD is not a valid guild ID; skipping view filtering.")
    ...
    async def _cache_enabled_servers(self):
        ...
        if environment == "custom":
            if not guildid:
                print("[❌] CUSTOM_GUILD not defined in .env; skipping enabled-server caching filter.")
            else:
                try:
                    query["_id"] = int(guildid)
                except ValueError:
                    print("[❌] CUSTOM_GUILD is not a valid guild ID; skipping enabled-server caching filter.")
