...
class Infractions(commands.Cog):
    ...
    async def infraction_multiple(
        ...
        *,
        ...
        await ctx.send(
            f" **{ctx.author.display_name}**, select the users you want to infraction!",
            view=view,
        )
    ...
    async def issue(
        ...
        *,
        ...
        msg = await ctx.send(
            content=f" **{ctx.author.display_name},** hold on while I infract this staff member.",
        )
        ...
        if Config.get("Module Options", {}).get("Infraction Confirmation", False):
            ...
            msg = await msg.edit(
                embeds=embeds,
                view=view,
                content=f" **{ctx.author.display_name}**, are you sure?\n-# Infraction Preview",
            )
            await view.wait()
    ...
    async def list(
        ...
        for i, infraction in enumerate(infractions):
            ...
            embed.add_field(
                name=f" Infraction | {infraction['random_string']} {voided}",
