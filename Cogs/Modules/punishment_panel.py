...
from utils.emojis import tick, no, greencheck, redx, hammer, page, info, modd, reason as reason_emoji, usr
...
WARN_MUTE_ROLE = 1431769854112698390
KICK_BAN_ROLE = 1431726641465393162
PUNISHMENT_WHITELIST_ROLE = 1429167267891118160
LOG_CHANNEL_ID = 1432451511266578452
IMAGE_STORAGE_CHANNEL_ID = 1446888231227494560
BAN_REQUEST_CHANNEL_ID = 1432451652408971335
ACTION_PROOFS_CHANNEL_ID = 1447086449462874204
...
class AttachProofsView(discord.ui.View):
    ...
    async def attach_proofs(
        ...
        try:
            ...
            # Inform user with embed (styled like the image)
            try:
                instr_embed = discord.Embed(
                    title="Attach Proofs",
                    description=f"I've created a message in <#{ACTION_PROOFS_CHANNEL_ID}> — please reply to that message with your image attachments.",
                    color=discord.Color.from_rgb(54, 57, 63),
                    timestamp=datetime.utcnow()
                )
                instr_embed.add_field(name="Punishment ID", value=f"`{self.punishment_id}`", inline=False)
                instr_embed.set_footer(text="Jump to message", icon_url=interaction.user.display_avatar)
                instr_embed_view = discord.ui.View()
                instr_embed_view.add_item(discord.ui.Button(label="Jump to Message", url=instr.jump_url, style=discord.ButtonStyle.link))
                await interaction.response.send_message(
                    embed=instr_embed,
                    view=instr_embed_view,
                    ephemeral=True,
                )
            except Exception:
                pass
...
class AttachProofsSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            try:
                await interaction.followup.send(
                    f"{tick} I've created a message in <#{ACTION_PROOFS_CHANNEL_ID}> — please reply to that message with your image attachments. Jump: {instr.jump_url}",
                    ephemeral=True,
                )
            except Exception:
                pass
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            if punishment_type in ["warn", "timeout"]:
                if WARN_MUTE_ROLE not in user_roles:
                    await interaction.response.send_message(
                ...
                # WARN_MUTE_ROLE can request bans
                if not any(role.id in [WARN_MUTE_ROLE, KICK_BAN_ROLE] for role in interaction.user.roles):
                    await interaction.response.send_message(
...
class PunishmentPanel(commands.Cog):
    ...
    @commands.check(lambda ctx: any(role.id == WARN_MUTE_ROLE for role in ctx.author.roles))
    @app_commands.check(lambda interaction: any(role.id == WARN_MUTE_ROLE for role in interaction.user.roles))
    async def warn_cmd(self, ctx: commands.Context, users: commands.Greedy[discord.User], *, reason: str = None):
        """Warn users. Usage: .warn @user1 @user2 ?r reason here"""
        try:
            # Limit to 5 users
    ...
    @commands.check(lambda ctx: any(role.id == KICK_BAN_ROLE for role in ctx.author.roles))
    @app_commands.check(lambda interaction: any(role.id == KICK_BAN_ROLE for role in interaction.user.roles))
    async def kick_cmd(self, ctx: commands.Context, users: commands.Greedy[discord.User], *, reason: str = None):
        """Kick users. Usage: .kick @user1 @user2 ?r reason here"""
        try:
            # Limit to 5 users
            users = users[:5] if users else []
            if not users:
    ...
    async def ban_cmd(self, ctx: commands.Context, users: commands.Greedy[discord.User], *, reason: str = None):
        """Ban users. Usage: .ban @user1 @user2 ?r reason here"""
        try:
            # Limit to 5 users
            users = users[:5] if users else []
            if not users:
    ...
    async def timeout_cmd(self, ctx: commands.Context, users: commands.Greedy[discord.User], duration: str, *, reason: str = None):
        """Timeout users. Usage: .timeout @user1 @user2 1h ?r reason here"""
        try:
            # Limit to 5 users
            users = users[:5] if users else []
            if not users:
        ...
        try:
            ...
            try:
                ...
                if log_channel:
                    ...
                    embed.set_footer(text=f"Timestamp: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}", icon_url=ctx.author.display_avatar)
                    await log_channel.send(embed=embed)
            except Exception as e:
    ...
    async def banrequest_cmd(self, ctx: commands.Context, users: commands.Greedy[discord.User], *, reason: str = None):
        """Request a ban for users. Usage: .banrequest @user1 @user2 ?r reason"""
        try:
            # Limit to 5 users
            users = users[:5] if users else []
            if not users:
    ...
    async def attach_proofs(
        ...
        if punishment["issuer_id"] != interaction.user.id:
            ...
            if not any(
                role.id in [WARN_MUTE_ROLE, KICK_BAN_ROLE]
    ...
    async def case_hybrid(self, ctx: commands.Context, user_or_id: Optional[str] = None):
        """View modcases: single case by ID, user's cases, or all server cases (paginated)"""
        try:
            ...
            if not user_or_id:
                ...
                title = f"{ctx.guild.name}'s Cases"
                ...
                async def build_page_all(page_num):
                    ...
                    embed.set_author(name=title, icon_url=ctx.author.display_avatar)
            ...
            if user:
                ...
                if not modcases:
                    ...
                    embed.set_author(name=f"{user.display_name}'s Cases", icon_url=user.display_avatar)
                ...
                embed = discord.Embed(
                    title=f"Punishment Issued | {case['punishment_id']}",
                    color=discord.Color.from_rgb(54, 57, 63),
                    timestamp=case['timestamp'],
                )

                embed.add_field(name="Punishment Type", value=case['punishment_type'].title(), inline=False)
                embed.add_field(name="Target User", value=f"<@{case['target_user_id']}>", inline=False)
                embed.add_field(name="Punished User ID", value=f"`{case['target_user_id']}`", inline=False)
                embed.add_field(name="Reason", value=case['reason'], inline=False)

                if case['punishment_type'] == 'timeout' and case.get('duration'):
                    embed.add_field(name="Duration", value=case['duration'], inline=False)

                embed.add_field(name="Issuer", value=f"<@{case['issuer_id']}>", inline=False)

                if case.get('proofs'):
                    proofs_text = "\n".join([f"[Proof {i+1}]({url})" for i, url in enumerate(case['proofs'])])
                    embed.add_field(name="Attached Proofs", value=proofs_text, inline=False)
    ...
    @commands.hybrid_command(name="modcases", aliases=["mymodcases", "mycases"])
    async def modcases_hybrid(self, ctx: commands.Context):
        """View modcases you have issued (paginated)"""
        try:
            # Query modcases issued by this user
            my_cases = list(await ctx.bot.qdb["punishments"].find(
                {"issuer_id": ctx.author.id, "guild_id": ctx.guild.id}
            ).sort("timestamp", -1).to_list(None))
            ...
            if not my_cases:
                ...
                embed.set_author(name=f"{ctx.author.display_name}'s modcases", icon_url=ctx.author.display_avatar)
                await ctx.send(embed=embed)
                return
