...
async def HandleButton(data: dict):
    ...
    for component in data.get("components", []):
        ...
        if component.get("type") == "link":
            # Ensure there is always a label for link buttons (Discord requires a label)
            label = (
                component.get("label")
                or component.get("name")
                or component.get("link")
                or "Link"
            )
            button = Button(
                label=label,
            ...
            if emoji:
                emoji = component.get("emoji")
            # Ensure there's a label (or emoji) so Discord doesn't reject the component.
            label = component.get("label") or component.get("name") or "Button"
            # Build a safe custom_id if none provided. Normalize spaces to underscores.
            raw_custom = component.get("custom_id") or label
            custom_id = (
                raw_custom.lower().replace(" ", "_") if raw_custom else None
            )
            button = Button(
                label=label,
...
class NoEmbed(discord.ui.View):
    ...
    @discord.ui.button(
        label="Button",
        style=discord.ButtonStyle.blurple,
        emoji = None if not IsSeperateBot() else None,
    )
    async def Buttons(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    async def Buttons(
        ...
        if self.typed == "Custom Commands":
            ...
            if comp and len(comp) > 0:
                ...
                embed.description = "There are no buttons yet. You can create one below by pressing the **plus button.**"
            print(self.data, interaction.user)
            view = componentmanager(interaction.user, self.data) # type: ignore
            print(view)
            await interaction.followup.send(view=view, embed=embed, ephemeral=True)
...
class CustomButton(discord.ui.Modal, title="Custom Button"):
    def __init__(self, data: dict):
        ...
        self.emoji = discord.ui.TextInput(
            label="Emoji",
            placeholder="What emoji should be on the button? (Example: )",
            required=False,
            max_length=256,
        )
...
class componentmanager(discord.ui.View):
    ...
    @discord.ui.button(
        label="Add Component", style=discord.ButtonStyle.gray
    )
    async def AddButton(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    async def RemoveButton(
        ...
        options = [
            discord.SelectOption(
                label=component.get("label") or component.get("name") or f"Button {i}",
                value=str(component.get("ix", i))
            )
            for i, component in enumerate(self.data.get("components", []))
        ][:25]
        view = discord.ui.View()
        ...
        await interaction.edit_original_response(
            view=view,
            embed=None,
            content=" Select which components you want to remove.",
        )
...
class EraseButtons(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            self.data["components"] = [
                c for c in self.data["components"] 
                if str(c.get("ix", "")) != str(selected_idx)
            ]
...
class EmbedFieldManager(discord.ui.View):
    ...
    @discord.ui.button(
        label="", style=discord.ButtonStyle.gray, emoji = None
    )
    async def add_field(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        label="", style=discord.ButtonStyle.blurple, emoji = None
    )
